...
{
    /// <summary>
    /// Socket 의 생명 주기 및 Socket 을 통한 데이터 송수신 처리
    /// </summary>
    public abstract class TcpSession : IDisposable
    {
        public TcpSession(Socket socket, int buffersize = 16 * KB)
        {
            ...
            _sendQueue = new ConcurrentQueue<ArraySegment<byte>>(); // 송신 대기열 초기화
        }
        ...
        private object _sendGate = new object();
        ...
        protected virtual void Send(byte[] body)
        {
            ...
            // 데이터 송신 시, body 길이를 포함한 헤더를 붙여서 보낼 거임.
            // Body 에 Header 붙인 길이
        ...
        private async Task SendAsync(ArraySegment<byte> segment)
        {
            try
            {
                int sent = 0; // 실제로 소켓에서 전송에 성공한 바이트 수

                // 내가 보내야 하는 세그먼트를 모두 보낼 때까지 반복
                while (sent < segment.Count)
                {
                    // 데이터를 전송하겠다고 해서 전부 한번에 전송보장이 되는 것이 아니다. 그래서 전송하고 남은 거 또 보내야한다.
                    ArraySegment<byte> remains = new ArraySegment<byte>(segment.Array, segment.Offset + sent, segment.Count - sent);

                    int justSent = await Socket.SendAsync(remains);
        ...
        private void ParsePackets()
        {
            ...
            while (true)
            {
                // 헤더 길이도 안 되는 데이터는 파싱이 안 되므로 데이터가 더 쌓일 때까지 기다림
                ...
                // body 가 완전하게 다 도착하지 않았다면 다 도착할 때까지 다시 검사
        ...
        {
            Dispose(true);
            GC.SuppressFinalize(this); // 가비지 컬렉션에 의해 이 객체의 소멸자가 호출되지 않도록 막음.
        }
